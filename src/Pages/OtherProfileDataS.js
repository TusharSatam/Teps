...
const OtherProfileDataS = ({ setNumber, user_id }) => {
  const { user, setstrategyNum } = useAuth();
  const [savedStrategy, setSavedStrategy] = useState([]);
  const [savedStrategyHindi, setSavedStrategyHindi] = useState([]);
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      getSaves().then((res) => {
        getLikes().then((res1) => {
          const myLikes = res1?.data?.filter(
            (ress) => ress.user_id === user?._id
          );
          const mySaveIdsArr = myLikes?.map((ress) => ress.strategie_id);
          setSaves(mySaveIdsArr);
        });
        const save = res?.data?.filter((ress) => ress.user_id === id);
        const mySaves = res?.data?.filter((ress) => ress.user_id === user?._id);
        const saveIdsArr = save?.map((ress) => ress.strategie_id);
        const mySaveIdsArr = mySaves?.map((ress) => ress.strategie_id);
        setLikes(mySaveIdsArr);
        if (languageSelect === "en") {
          setIsLoading(true);
          getMultitStr(saveIdsArr)
            .then((res) => {
          ...
          getMultitHiStr(saveIdsArr)
            .then((res) => {
              setSavedStrategyHindi(res?.data);
              getMultiUserHindiStr(saveIdsArr)
                .then((res2) => {
                  setSavedStrategyHindi((prev) => [...prev, ...res2?.data]);
                  setIsLoading(false);
                })
                .catch((err2) => {
                  console.log({ err2 });
  ...
  }, [savedStrategy, savedStrategyHindi, languageSelect]);
  useEffect(() => {
    console.log({ likes });
  }, [likes]);
  return (
      ...
      {languageSelect === "en" ? (
        <>
          ...
          {isLoading && !collapse ? (
            ...
            <div>
              {savedStrategy?.slice(0, displayCount).map((res, index) => (
              ...
              {!showAll && savedStrategy?.length > 2 ? (
                <div
                  style={{
                    display: "flex",
        ...
        <>
          ...
          ) : savedStrategyHindi?.length === 0 && collapse !== true ? (
            <h1 className="my-5 text-center py-5 text-danger">
              {t("No Saved Strategies available.")}
            </h1>
          ) : savedStrategyHindi?.length !== 0 && collapse !== true ? (
            <div>
              {savedStrategyHindi?.slice(0, displayCount).map((res, index) => (
                <div key={index} className="cardContainer">
              ...
              {!showAll && savedStrategyHindi.length > 2 ? (
                <div
                  style={{
                    display: "flex",
                    justifyContent: "flex-end",
...
export default OtherProfileDataS;