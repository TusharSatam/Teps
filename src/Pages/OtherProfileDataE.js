...
import { getHindiStratigysEditedbyUser } from "../services/hindiStratigys";
import {
  getLikes,
  postLikes,
...
const OtherProfileDataE = ({ setNumber }) => {
  const { user, setstrategyNum } = useAuth();
  const [editedStrategy, setEditedStrategy] = useState([]);
  const [editedStrategyHindi, setEditedStrategyHindi] = useState([]);
    ...
    if (
      ...
      getLikes().then((res) => {
        ...
        if (languageSelect === "en") {
          ...
          getEdits(id)
        .then((res) => {
          // const edits = res?.data?.filter((ress) => ress.Approve === true);
          setEditedStrategy(res?.data);
          setIsLoading(false);
        })
        .catch((err) => {
          setEditedStrategy([]);
          ...
          getHindiStratigysEditedbyUser(id)
        .then((res) => {
          // console.log({ res });
          setEditedStrategyHindi(res);
          setIsLoading(false);
        })
        .catch((err) => {
          setEditedStrategyHindi([]);
          console.log({ err });
          setIsLoading(false);
        });
        }
  ...
  const displayCount = showAll
    ? languageSelect === "en"
      ? editedStrategy?.length
      : editedStrategyHindi?.length
    : 2;
  React.useEffect(() => {
    if (languageSelect === "en") {
      setNumber(editedStrategy?.length);
    }
    if (languageSelect === "hi") {
      setNumber(editedStrategyHindi?.length);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [editedStrategy, editedStrategyHindi, languageSelect]);
  useEffect(() => {
    console.log({ likes });
  }, [likes]);
  return (
    <div>
      {languageSelect === "en" ? (
        <>
          ...
          ) : editedStrategy?.length === 0 && collapse !== true ? (
            <h1 className="my-5 text-center py-5 text-danger">
              {t("No edited strategies available.")}
            </h1>
          ) : editedStrategy?.length !== 0 && collapse !== true ? (
            <div>
              {editedStrategy?.slice(0, displayCount).map((res, index) => (
              ...
              {!showAll && editedStrategy?.length > 2 ? (
                <div
                  style={{
                    display: "flex",
        ...
        <>
          ...
          ) : editedStrategyHindi?.length === 0 && collapse !== true ? (
            <h1 className="my-5 text-center py-5 text-danger">
              {t("No Edited Strategies available.")}
            </h1>
          ) : editedStrategyHindi?.length !== 0 && collapse !== true ? (
            <div>
              {editedStrategyHindi?.slice(0, displayCount).map((res, index) => (
                <div key={index} className="cardContainer">
              ...
              {!showAll && editedStrategyHindi.length > 2 ? (
                <div
                  style={{
                    display: "flex",
                    justifyContent: "flex-end",
...
export default OtherProfileDataE;
