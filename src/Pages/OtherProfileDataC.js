...
import { getHindiStratigysCreatedByUser } from "../services/hindiStratigys";
import {
  getLikes,
  postLikes,
...
const OtherProfileDataC = ({ setNumber }) => {
  const { user, setstrategyNum } = useAuth();
  const [createdStrategy, setCreatedStrategy] = useState([]);
  const [createdStrategyHindi, setCreatedStrategyHindi] = useState([]);
    ...
    if (
      ...
      getLikes().then((res) => {
        ...
        if (languageSelect === "en") {
          ...
          getUserCreated(id)
        .then((res) => {
          // const created = res?.data?.filter((ress) => ress.Approve === true);
          setCreatedStrategy(res?.data);
          setIsLoading(false);
        })
        .catch((err) => {
          setCreatedStrategy([]);
          ...
          setIsLoading(false);
            ...
            getHindiStratigysCreatedByUser(id)
        .then((res) => {
          // console.log({ res });
          setCreatedStrategyHindi(res);
          setIsLoading(false);
        })
        .catch((err) => {
          setCreatedStrategyHindi([]);
          console.log({ err });
          setIsLoading(false);
        });
        }
  ...
  const displayCount = showAll
    ? languageSelect === "en"
      ? createdStrategy?.length
      : createdStrategyHindi?.length
    : 2;
  React.useEffect(() => {
    if (languageSelect === "en") {
      setNumber(createdStrategy?.length);
    }
    if (languageSelect === "hi") {
      setNumber(createdStrategyHindi?.length);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [createdStrategy, createdStrategyHindi, languageSelect]);
  useEffect(() => {
    console.log({ likes });
  }, [likes]);
  return (
    <div>
      {languageSelect === "en" ? (
        <>
          ...
          ) : createdStrategy?.length === 0 && collapse !== true ? (
            <h1 className="my-5 text-center py-5 text-danger">
              {t("No created strategies available.")}
            </h1>
          ) : createdStrategy?.length !== 0 && collapse !== true ? (
            <div>
              {createdStrategy?.slice(0, displayCount).map((res, index) => (
              ...
              {!showAll && createdStrategy?.length > 2 ? (
                <div
                  style={{
                    display: "flex",
        ...
        <>
          ...
          ) : createdStrategyHindi?.length === 0 && collapse !== true ? (
            <h1 className="my-5 text-center py-5 text-danger">
              {t("No Created Strategies available.")}
            </h1>
          ) : createdStrategyHindi?.length !== 0 && collapse !== true ? (
            <div>
              {createdStrategyHindi?.slice(0, displayCount).map((res, index) => (
                <div key={index} className="cardContainer">
              ...
              {!showAll && createdStrategyHindi.length > 2 ? (
                <div
                  style={{
                    display: "flex",
                    justifyContent: "flex-end",
...
export default OtherProfileDataC;
